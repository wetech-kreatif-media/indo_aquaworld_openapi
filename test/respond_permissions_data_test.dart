import 'package:test/test.dart';
import 'package:openapi/openapi.dart';

// tests for RespondPermissionsData
void main() {
  final instance = RespondPermissionsDataBuilder();
  // TODO add properties to the builder and call build()

  group(RespondPermissionsData, () {
    // bool last
    test('to test the property `last`', () async {
      // TODO
    });

    // bool first
    test('to test the property `first`', () async {
      // TODO
    });

    // bool empty
    test('to test the property `empty`', () async {
      // TODO
    });

    // num size
    test('to test the property `size`', () async {
      // TODO
    });

    // num number
    test('to test the property `number`', () async {
      // TODO
    });

    // num totalPages
    test('to test the property `totalPages`', () async {
      // TODO
    });

    // num totalElements
    test('to test the property `totalElements`', () async {
      // TODO
    });

    // num numberOfElements
    test('to test the property `numberOfElements`', () async {
      // TODO
    });

    // BuiltList<RespondPermissionsDataContentInner> content
    test('to test the property `content`', () async {
      // TODO
    });

  });
}
